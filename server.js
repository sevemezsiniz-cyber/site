...
const requireLogin = (req, res, next) => {
    ...
    if (!user || user.status === 'banned') {
        req.session.destroy();
        return res.redirect('/giris?error=Hesabınız banlanmış veya geçersiz');
    }
    if (user.role !== 'admin' && user.status !== 'approved') {
        req.session.destroy();
        return res.redirect('/giris?error=Hesabınız henüz admin tarafından onaylanmadı');
    }
    next();
};
...
app.post('/giris', async (req, res) => {
    ...
    if (user.status === 'banned') return res.redirect('/giris?error=Bu hesap banlanmış');
    if (user.role !== 'admin' && user.status !== 'approved') return res.redirect('/giris?error=Hesabınız henüz onaylanmadı');
...
app.post('/kayit', async (req, res) => {
    ...
    saveUsers();
    res.redirect('/giris?error=Kayıt başarılı! Hesabınız admin onayı bekliyor.');
});

// ADMIN PANELİ
app.get('/admin', requireAdmin, (req, res) => {
    const pendingUsers = Object.keys(users).filter(u => users[u].status === 'pending' && users[u].role !== 'admin');
    const approvedUsers = Object.keys(users).filter(u => users[u].status === 'approved' && users[u].role !== 'admin');
    const bannedUsers = Object.keys(users).filter(u => users[u].status === 'banned' && users[u].role !== 'admin');
    ...
    <head>
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            ...
            .header{background:linear-gradient(135deg,#000000,#1a0000);padding:20px;border-radius:20px;margin-bottom:30px;border:1px solid rgba(255,0,0,0.3);}
            h1{color:#ff3333;}
            .logout{float:right;background:#dc3545;color:white;padding:8px 20px;text-decoration:none;border-radius:25px;}
            .admin-badge{background:#ff3333;color:white;padding:5px 15px;border-radius:20px;display:inline-block;margin-left:15px;font-size:14px;}
            .section{background:rgba(0,0,0,0.85);border-radius:20px;padding:25px;margin-bottom:30px;border:1px solid rgba(255,0,0,0.3);}
            .section h2{color:#667eea;margin-bottom:20px;border-bottom:1px solid #333;padding-bottom:10px;}
            table{width:100%;border-collapse:collapse;color:white;}
            th,td{padding:12px;text-align:left;border-bottom:1px solid #333;}
            th{background:#1a1a2e;color:#ff3333;}
            ...
            .unban{background:#ffc107;color:black;}
            .view-queries{background:#17a2b8;color:white;}
            .queries-box{background:#0a0a0a;padding:15px;border-radius:10px;margin-top:10px;max-height:300px;overflow-y:auto;font-family:monospace;font-size:12px;white-space:pre-wrap;}
            details{margin-top:10px;}
            summary{cursor:pointer;color:#667eea;margin:10px 0;}
        </style>
    </head>
    <body>
        <div class="container">
            <div class="header">
                ...
                <h1>Admin Paneli <span class="admin-badge">${req.session.userId}</span></h1>
                <p style="color:#667eea;margin-top:10px;">Kullanıcıları yönet, sorgularını gör</p>
            </div>
            
            <div class="section">
                <h2>⏳ Onay Bekleyenler (${pendingUsers.length})</h2>
                ${pendingUsers.length === 0 ? '<p style="color:#888;">Bekleyen kullanıcı yok</p>' : `
                <table>
                    <tr><th>Kullanıcı</th><th>İşlemler</th></tr>
                    ${pendingUsers.map(u => `
                        <tr>
                            <td>${u}</td>
                            <td>
                                <button class="approve" onclick="actionUser('${u}','approve')">✅ Onayla</button>
                                <button class="ban" onclick="actionUser('${u}','ban')">🔨 Banla</button>
                            </td>
                        </tr>
                    `).join('')}
                </table>
            ...
            <div class="section">
                <h2>✅ Onaylı Kullanıcılar (${approvedUsers.length})</h2>
                ${approvedUsers.length === 0 ? '<p style="color:#888;">Onaylı kullanıcı yok</p>' : `
                <table>
                    <tr><th>Kullanıcı</th><th>Sorgu Sayısı</th><th>İşlemler</th></tr>
                    ${approvedUsers.map(u => `
                        <tr>
                            <td>${u}</td>
                            <td>${users[u].queries?.length || 0}</td>
                            <td>
                                <button class="ban" onclick="actionUser('${u}','ban')">🔨 Banla</button>
                                <button class="view-queries" onclick="showQueries('${u}')">📋 Sorguları Gör</button>
                            </td>
                        </tr>
                    `).join('')}
                </table>
            ...
            <div class="section">
                <h2>🚫 Banlı Kullanıcılar (${bannedUsers.length})</h2>
                ${bannedUsers.length === 0 ? '<p style="color:#888;">Banlı kullanıcı yok</p>' : `
                <table>
                    <tr><th>Kullanıcı</th><th>İşlemler</th></tr>
                    ${bannedUsers.map(u => `
                        <tr>
                            <td>${u}</td>
                            <td>
                                <button class="unban" onclick="actionUser('${u}','unban')">🔓 Banı Kaldır</button>
                            </td>
                        </tr>
                    `).join('')}
                </table>
            ...
            <div id="queriesModal" style="display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#1a1a2e;padding:25px;border-radius:20px;max-width:800px;width:90%;max-height:80%;overflow-y:auto;z-index:1000;border:2px solid #ff3333;">
                <h3 style="color:#ff3333;" id="modalTitle">Sorgular</h3>
                <div id="modalContent" class="queries-box"></div>
                <button onclick="closeModal()" style="margin-top:15px;background:#dc3545;color:white;padding:8px 20px;border:none;border-radius:10px;">Kapat</button>
            </div>
            <div id="overlay" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:999;" onclick="closeModal()"></div>
        </div>
        <script>
        ...
        async function showQueries(username) {
            const res = await fetch('/admin/queries/'+username);
            const data = await res.json();
            document.getElementById('modalTitle').innerHTML = '📋 ' + username + ' - Sorgu Geçmişi';
            if(data.queries && data.queries.length > 0) {
                let html = '';
                data.queries.forEach((q, i) => {
                    html += '<strong>' + (i+1) + '. ' + new Date(q.time).toLocaleString('tr-TR') + '</strong><br>';
                    html += 'Sorgu: ' + q.query + '<br>';
                    html += 'Sonuç: ' + q.result.substring(0, 500) + (q.result.length > 500 ? '...' : '') + '<br><hr>';
                });
                document.getElementById('modalContent').innerHTML = html;
            } else {
                document.getElementById('modalContent').innerHTML = 'Henüz sorgu yapılmamış.';
            }
...
app.post('/admin/approve', requireAdmin, (req, res) => {
    ...
    if (!users[username] || users[username].role === 'admin') return res.json({ success: false, error: 'Geçersiz kullanıcı' });
...
app.post('/admin/ban', requireAdmin, (req, res) => {
    const { username } = req.body;
    if (!users[username] || users[username].role === 'admin') return res.json({ success: false, error: 'Admin banlanamaz' });
...
app.post('/admin/unban', requireAdmin, (req, res) => {
    const { username } = req.body;
    if (!users[username]) return res.json({ success: false, error: 'Kullanıcı bulunamadı' });
    users[username].status = 'approved';
    saveUsers();
    res.json({ success: true });
});

app.get('/admin/queries/:username', requireAdmin, (req, res) => {
    const { username } = req.params;
    if (!users[username]) return res.json({ queries: [] });
    res.json({ queries: users[username].queries || [] });
});

app.get('/dashboard', requireLogin, (req, res) => {
    const isAdmin = users[req.session.userId]?.role === 'admin';
    res.send(`
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            ...
            .admin-btn{float:right;background:#ff3333;color:white;padding:8px 20px;text-decoration:none;border-radius:25px;font-weight:bold;margin-right:10px;}
    ...
    <body>
        <div class="header">
            <a href="/cikis" class="logout">Çıkış</a>
            ${isAdmin ? '<a href="/admin" class="admin-btn">👑 Admin Paneli</a>' : ''}
            <h1>SEVEMEZSİNİZ</h1>
...
// Sorgu kaydetme fonksiyonu
async function logQuery(username, queryType, queryParams, result) {
    if (!users[username] || users[username].role === 'admin') return;
    if (!users[username].queries) users[username].queries = [];
    users[username].queries.push({
        time: new Date().toISOString(),
        query: `${queryType}?${queryParams}`,
        result: result.substring(0, 2000) // Çok uzun sonuçları kısalt
    });
    // Son 100 sorguyu tut
    if (users[username].queries.length > 100) users[username].queries.shift();
    saveUsers();
}
...
apiList.forEach(api => {
    app.get(`/sorgu/${api.id}`, requireLogin, (req, res) => {
        ...
        <div class="container">
            ...
            <h2>${api.name}</h2>
            <form id="sorguForm" onsubmit="sorgula(event)">
                ${api.type==='ad'?`<input id="name" placeholder="İsim" required><input id="surname" placeholder="Soyisim" required>`:`<input id="input" placeholder="${api.type==='tc'?'TC Kimlik No':api.type==='gsm'?'GSM No':'Plaka'}" required>`}
...
app.get('/api/proxy/:endpoint', async (req, res) => {
    ...
    try {
        ...
        // Sorguyu logla (admin değilse)
        if (username && users[username] && users[username].role !== 'admin') {
            await logQuery(username, endpoint, new URLSearchParams(req.query).toString(), resultText.substring(0, 2000));
        }
