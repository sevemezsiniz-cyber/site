...
app.get('/dashboard', requireLogin, (req, res) => {
    ...
    <body>
        <div class="header">
            <a href="/cikis" class="logout">Cikis</a>
            <h1>SEVEMEZSINIZ</h1>
    ...
    app.get(`/sorgu/${api.id}`, requireLogin, (req, res) => {
        if (api.id === 'query') {
            ...
            <style>
                *{margin:0;padding:0;box-sizing:border-box;}
                ...
                .container{max-width:800px;margin:0 auto;background:rgba(0,0,0,0.85);padding:30px;border-radius:20px;border:1px solid rgba(255,0,0,0.3);}
                ...
                button{background:linear-gradient(90deg,#ff3333,#cc0000);font-weight:bold;cursor:pointer;}
                .result-box,.error-box{display:none;padding:15px;border-radius:10px;margin-top:20px;white-space:pre-wrap;word-wrap:break-word;overflow-x:auto;max-height:500px;overflow-y:auto;}
                .result-box{background:#0a0a0a;border-left:4px solid #28a745;color:#00ff88;font-family:monospace;font-size:13px;}
            ...
            async function sorgula(e){
                ...
                try{
                    ...
                    const filteredLines=lines.filter(line=>line.toLowerCase().includes(city.toLowerCase()));
                    if(filteredLines.length===0){
                        errorBox.textContent='"'+city+'" icin eslesen kayit bulunamadi';
                        errorBox.style.display='block';
                    }else{
                        let sonuc = '🎯 "' + city + '" icin bulunan sonuclar (' + filteredLines.length + ' kayit):\\n\\n';
                        sonuc += '==========================================\\n\\n';
                        for(let i = 0; i < filteredLines.length; i++) {
                            sonuc += (i+1) + '. ' + filteredLines[i] + '\\n\\n';
                        }
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            ...
            h2{color:#ff3333;margin-bottom:20px;}
            input,button{width:100%;padding:12px;margin:8px 0;background:#1a1a2e;border:1px solid #333;border-radius:10px;color:white;}
            button{background:linear-gradient(90deg,#667eea,#764ba2);font-weight:bold;cursor:pointer;}
            .result-box,.error-box{display:none;padding:15px;border-radius:10px;margin-top:20px;white-space:pre-wrap;word-wrap:break-word;overflow-x:auto;max-height:500px;overflow-y:auto;}
            .result-box{background:#0a0a0a;border-left:4px solid #667eea;color:#00ff88;font-family:monospace;font-size:13px;}
            .error-box{background:rgba(255,0,0,0.2);border-left:4px solid #ff3333;color:#ff6666;}
        </style>
        </head>
        <body>
        <div class="container">
            ...
            <h2>${api.name}</h2>
            <form id="sorguForm" onsubmit="sorgula(event)">
                ${api.type==='ad'?`<input id="name" placeholder="Isim" required><input id="surname" placeholder="Soyisim" required>`:`<input id="input" placeholder="${api.type==='tc'?'TC Kimlik No':api.type==='gsm'?'GSM No':'Plaka'}" required>`}
        ...
        async function sorgula(e){
            ...
            }catch(err){errorBox.textContent='Baglanti Hatasi: '+err.message;errorBox.style.display='block';}
        }
        </script>
        </body>
        </html>
...
app.get('/api/proxy/:endpoint', async (req, res) => {
    ...
    try {
        ...
        else if (err.request) res.status(503).send('API servisine ulasilamiyor');
        else res.status(500).send('Sunucu Hatasi: ' + err.message);
    }
});
