    ...
    <head>
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            body{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(135deg,#0a0a0a 0%,#1a1a2e 100%);min-height:100vh;display:flex;justify-content:center;align-items:center;padding:20px;}
            .container{background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);border-radius:20px;padding:40px;max-width:450px;width:100%;border:1px solid rgba(255,0,0,0.3);box-shadow:0 0 40px rgba(255,0,0,0.2);}
            h2{text-align:center;color:#ff3333;font-size:28px;margin-bottom:10px;}
            .sub{text-align:center;color:#667eea;margin-bottom:30px;font-size:12px;}
            ...
            .error{background:rgba(255,0,0,0.2);border:1px solid #ff3333;color:#ff6666;padding:10px;border-radius:8px;text-align:center;margin-bottom:15px;}
...
app.get('/kayit', (req, res) => {
    ...
    <head>
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            body{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(135deg,#0a0a0a 0%,#1a1a2e 100%);min-height:100vh;display:flex;justify-content:center;align-items:center;padding:20px;}
            .container{background:rgba(0,0,0,0.85);backdrop-filter:blur(10px);border-radius:20px;padding:40px;max-width:450px;width:100%;border:1px solid rgba(0,255,255,0.3);}
            h2{text-align:center;color:#667eea;font-size:28px;margin-bottom:10px;}
...
app.get('/dashboard', requireLogin, (req, res) => {
    ...
    <head>
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            body{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(135deg,#0a0a0a 0%,#1a1a2e 100%);min-height:100vh;padding:20px;}
    ...
    app.get(`/sorgu/${api.id}`, requireLogin, (req, res) => {
        if (api.id === 'query') {
            ...
            <style>
                *{margin:0;padding:0;box-sizing:border-box;}
                body{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(135deg,#0a0a0a 0%,#1a1a2e 100%);min-height:100vh;padding:20px;}
                .container{max-width:650px;margin:0 auto;background:rgba(0,0,0,0.85);padding:30px;border-radius:20px;border:1px solid rgba(255,0,0,0.3);}
                ...
                .result-box{background:#0a0a0a;border-left:4px solid #28a745;color:#00ff88;}
            ...
            async function sorgula(e){
                ...
                try{
                    ...
                    const filteredLines=lines.filter(line=>line.includes(city));
                    if(filteredLines.length===0){
                        errorBox.textContent='"' + city + '" için eşleşen kayıt bulunamadı.';
                        errorBox.style.display='block';
                    }else{
                        resultBox.textContent='"'+city+'" için bulunan sonuçlar:\\n\\n'+filteredLines.join('\\n');
        ...
        <style>
            *{margin:0;padding:0;box-sizing:border-box;}
            ...
            .result-box,.error-box{display:none;padding:15px;border-radius:10px;margin-top:20px;white-space:pre-wrap;}
            .result-box{background:#0a0a0a;border-left:4px solid #667eea;color:#00ff88;}
            .error-box{background:rgba(255,0,0,0.2);border-left:4px solid #ff3333;color:#ff6666;}
        </style>
        </head>
        <body>
...
app.get('/api/proxy/:endpoint', async (req, res) => {
    ...
    try {
        ...
        if (endpoint === 'soyagaci') {
            ...
            if (!tc) throw new Error("TC kimlik numarası gereklidir.");
        ...
        console.error(`Proxy hatası (${endpoint}):`, err.message);
