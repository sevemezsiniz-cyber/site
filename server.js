...
// --- API listesi (tüm endpoint'ler eklendi) ---
const apiList = [
    // Mevcut API'leriniz (hepsi burada)
    ...
    // --- DEĞİŞTİRİLEN KISIM: Kapsamlı Sorgu için tip 'ozel' yapıldı ---
    ...
    // --- YENİ EKLENEN API: Sülale Ağacı (Bedava API'den) ---
    { id: 'soyagaci', name: '🌳 Soy Ağacı (TC ile)', type: 'tc', api: 'soyagaci' } 
];

// --- Dashboard (Görsel olarak değişen bir şey yok, sadece yeni buton eklendi) ---
app.get('/dashboard', requireLogin, (req,res)=>{
    ...
    .api-btn.ozel{border-left:5px solid #ff00cc;}
...
// --- Sorgu Sayfaları (Kapsamlı Sorgu için özel form eklendi) ---
apiList.forEach(api=>{
    app.get(`/sorgu/${api.id}`,requireLogin,(req,res)=>{
        // --- KAPSAMLI SORGU İÇİN ÖZEL FORM ---
        if(api.id === 'query') {
            res.send(`
            ...
            input,button{width:100%;padding:10px;margin:5px 0;border-radius:5px;box-sizing:border-box;}
            ...
            async function sorgula(e){
                ...
                try{
                    ...
                    // Gelen sonuçları satır satır filtrele
                    const lines = text.split('\\n');
                    // Sadece girilen il adını içeren satırları al
                    const filteredLines = lines.filter(line => line.includes(city));
                    
                    if(filteredLines.length === 0) {
                        errorBox.textContent = 'İstanbul için eşleşen kayıt bulunamadı.';
            ...
            `);
            return; // Bu özel durum için fonksiyonu burada bitir.
        }

        // --- DİĞER TÜM SORGULAR (standart) ---
        res.send(`
        <!DOCTYPE html><html><head><title>${api.name}</title><meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
        body{font-family:Arial;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);min-height:100vh;padding:20px;}
...
// --- PROXY API (Yeni soy ağacı endpoint'i için özel yönlendirme eklendi) ---
app.get('/api/proxy/:endpoint', async (req,res)=>{
    const {endpoint}=req.params;
    try{
        let url;
        // Yeni eklenen soy ağacı sorgusu için özel yönlendirme
        if(endpoint === 'soyagaci') {
            const tc = req.query.tc;
            if(!tc) throw new Error("TC kimlik numarası gereklidir.");
            // https://bedavaapimorg.rf.gd/XD/sulale.php?tc=11111111110
            url = `https://bedavaapimorg.rf.gd/XD/sulale.php?tc=${encodeURIComponent(tc)}`;
        } 
        // Diğer tüm sorgular için mevkin API'niz
        else {
        ...
        else if(err.request) res.status(503).send('API servisine ulaşılamıyor veya yanıt vermiyor.');
        else res.status(500).send('Sunucu Hatası: '+err.message);
    }
});

// --- Giriş, Kayıt, Çıkış (Değişmedi) ---
app.get('/', (req, res) => {
